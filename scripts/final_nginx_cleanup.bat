@echo off
echo ========================================
echo Crys Garage - Final Nginx Cleanup
echo ========================================
...
echo Step 2: Removing ALL Nginx configs...
ssh root@209.74.80.162 "rm -f /etc/nginx/conf.d/*"
ssh root@209.74.80.162 "rm -f /etc/nginx/default.d/*"
ssh root@209.74.80.162 "rm -f /etc/nginx/sites-enabled/*"
echo ✅ All configs removed
echo.

echo Step 3: Creating minimal Nginx config...
ssh root@209.74.80.162 "echo 'server { listen 80; root /var/www/crysgarage-frontend; index index.html; location / { try_files \$uri \$uri/ /index.html; } }' > /etc/nginx/conf.d/default.conf"
