# Crys Garage Simple VPS Deployment Script (PowerShell) - AlmaLinux 8
...
Write-Host "🚀 Starting SIMPLE FRESH Crys Garage Deployment to AlmaLinux 8 VPS" -ForegroundColor $Green
...
function Check-Requirements {
    ...
    try {
        ...
        Write-Host "❌ scp not found. Please install OpenSSH or use Git Bash." -ForegroundColor $Red
        exit 1
    }
...
function Test-VPSConnection {
    ...
    try {
        $testResult = ssh -p $VPS_PORT -o ConnectTimeout=10 "${VPS_USER}@${VPS_HOST}" "echo 'Connection successful'; cat /etc/os-release | grep PRETTY_NAME" 2>$null
...
# Deploy backend using scp
function Deploy-Backend {
    Write-Host "Deploying fresh Laravel backend..." -ForegroundColor $Yellow
    
    # Create a temporary directory for backend files
    $tempBackendDir = "temp_backend_deploy"
    if (Test-Path $tempBackendDir) {
        Remove-Item -Recurse -Force $tempBackendDir
    }
    New-Item -ItemType Directory -Path $tempBackendDir | Out-Null
    
    # Copy backend files to temp directory, excluding unnecessary files
    Copy-Item -Path "crysgarage-backend\*" -Destination $tempBackendDir -Recurse -Force
    Remove-Item -Path "$tempBackendDir\vendor" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\node_modules" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\.env" -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\storage\logs" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\storage\framework\cache" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\storage\framework\sessions" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\storage\framework\views" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\bootstrap\cache" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\.git" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempBackendDir\tests" -Recurse -Force -ErrorAction SilentlyContinue
    
    # Deploy using scp
    scp -r -P $VPS_PORT "$tempBackendDir\*" "${VPS_USER}@${VPS_HOST}:${VPS_PATH}/crysgarage-backend/"
...
function Deploy-Frontend {
    ...
    # Deploy built files using scp
    scp -r -P $VPS_PORT "crysgarage-frontend\dist\*" "${VPS_USER}@${VPS_HOST}:${VPS_PATH}/crysgarage-frontend/"
    
    Write-Host "✅ Fresh frontend deployed" -ForegroundColor $Green
}

# Deploy Ruby audio processor using scp
function Deploy-Ruby {
    Write-Host "Deploying fresh Ruby audio processor..." -ForegroundColor $Yellow
    
    # Create a temporary directory for Ruby files
    $tempRubyDir = "temp_ruby_deploy"
    if (Test-Path $tempRubyDir) {
        Remove-Item -Recurse -Force $tempRubyDir
    }
    New-Item -ItemType Directory -Path $tempRubyDir | Out-Null
    
    # Copy Ruby files to temp directory, excluding unnecessary files
    Copy-Item -Path "crysgarage-ruby\*" -Destination $tempRubyDir -Recurse -Force
    Remove-Item -Path "$tempRubyDir\output" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempRubyDir\logs" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path "$tempRubyDir\temp" -Recurse -Force -ErrorAction SilentlyContinue
    
    # Deploy using scp
    scp -r -P $VPS_PORT "$tempRubyDir\*" "${VPS_USER}@${VPS_HOST}:${VPS_PATH}/crysgarage-ruby/"
...
function Main {
    ...
    Write-Host "🎉 SIMPLE FRESH Deployment completed successfully!" -ForegroundColor $Green
