...
ssh ${SERVER_USER}@${SERVER_IP} "mkdir -p ${REMOTE_DIR}/api && mkdir -p ${REMOTE_DIR}/storage && mkdir -p ${REMOTE_DIR}/bootstrap/cache"
...
ssh ${SERVER_USER}@${SERVER_IP} "chown -R nginx:nginx ${REMOTE_DIR}/api && chmod -R 755 ${REMOTE_DIR}/api && chmod -R 775 ${REMOTE_DIR}/api/storage && chmod -R 775 ${REMOTE_DIR}/api/bootstrap/cache"

# Step 6: Clear Laravel caches
Write-Host "🧹 Clearing Laravel caches..." -ForegroundColor Yellow
ssh ${SERVER_USER}@${SERVER_IP} "cd ${REMOTE_DIR}/api && php artisan config:clear && php artisan route:clear && php artisan cache:clear"
