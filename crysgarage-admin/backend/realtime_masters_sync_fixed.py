    ...
    async def sync_masters(self):
        ...
        try:
            ...
            async with AsyncSession(engine) as admin_session:
                ...
                # Query for recent audio files (all statuses)
                query = """
                ...
                # Use last sync time or 30 days ago as fallback to get more data
                since_time = self.last_sync_time or (datetime.now() - timedelta(days=30))
                ...
                for audio in recent_audio:
                    ...
                    if status == "processed":
                        ...
                        master_status = "uploaded"  # For uploaded files
                    ...
                    if existing_master:
                        ...
                        if master_status == "completed":
                            existing_master.completed_at = datetime.strptime(updated_at, '%Y-%m-%d %H:%M:%S') if updated_at else datetime.utcnow()
                        elif master_status == "uploaded":
                            existing_master.completed_at = None  # No completion time for uploaded files
                        ...
                        new_master = Master(
                            ...
                            created_at=datetime.strptime(created_at, '%Y-%m-%d %H:%M:%S') if created_at else datetime.utcnow(),
                            completed_at=datetime.strptime(updated_at, '%Y-%m-%d %H:%M:%S') if updated_at and master_status == "completed" else None
                        )
                ...
                # Also sync uploads
                upload_query = """
                SELECT id, user_id, file_size, created_at, original_filename
                FROM audio 
                ...
                for upload in recent_uploads:
                    audio_id, user_id, file_size, created_at, original_filename = upload
