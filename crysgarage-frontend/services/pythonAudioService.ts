...
class PythonAudioService {
   ...
   * Updated to match documentation: sends FormData to /api/v1/master
   */
  async processAudio(request: MasteringRequest): Promise<MasteringResponse> {
    try {
      ...
      // If request has file_url, we need to fetch it first and create a File object
      // Otherwise, if it's already a File, use it directly
      if (request.file_url) {
        // Fetch the file from URL and create a File object
        const fileResponse = await fetch(request.file_url);
        const blob = await fileResponse.blob();
        const fileName = request.file_url.split('/').pop() || 'audio.wav';
        const file = new File([blob], fileName, { type: blob.type });
        formData.append('file', file);
      } else if (request.file instanceof File) {
        formData.append('file', request.file);
      } else {
        throw new Error('File is required for mastering');
      }
      
      formData.append('tier', request.tier || 'professional');
      formData.append('genre', request.genre || 'default');
      formData.append('format', request.target_format || 'wav');
      formData.append('sample_rate', String(request.target_sample_rate || 44100));
      
      // Use /api/v1/master endpoint as per documentation
      const response = await axios.post(`${this.baseURL}/api/v1/master`, formData, {
        headers: {
  ...
  async uploadAndProcessAudio(
    ...
    try {
      ...
      if (isLocal) {
        ...
        console.log('Uploading file directly to Python for mastering (local, using /api/v1/master)...');
        const advData = new FormData();
        // Use /api/v1/master endpoint as per documentation
        advData.append('file', file);                    // Audio file to be processed
        advData.append('genre', genre);                   // Audio genre for processing
        advData.append('tier', tier === 'pro' ? 'professional' : tier); // Processing tier
        advData.append('format', format.toUpperCase()); // Output format as per documentation
        advData.append('sample_rate', String(sampleRate)); // Output sample rate as per documentation
        if (typeof targetLufs === 'number') {
          advData.append('target_lufs', String(targetLufs)); // Target loudness level
        }
        advData.append('user_id', userId);               // User identifier
        const resp = await axios.post(`${this.baseURL}/api/v1/master`, advData);
        ...
        console.log('Uploading file directly to Python for mastering (prod, using /api/v1/master)...');
        const advData2 = new FormData();
        // Use /api/v1/master endpoint as per documentation
        advData2.append('file', file);                    // Audio file to be processed
        advData2.append('genre', genre);                   // Audio genre for processing
        advData2.append('tier', tier === 'pro' ? 'professional' : tier); // Processing tier
        advData2.append('format', format.toUpperCase()); // Output format as per documentation
        advData2.append('sample_rate', String(sampleRate)); // Output sample rate as per documentation
        if (typeof targetLufs === 'number') {
          advData2.append('target_lufs', String(targetLufs)); // Target loudness level
        }
        advData2.append('user_id', userId);               // User identifier
        const resp = await axios.post(`${this.baseURL}/api/v1/master`, advData2);
