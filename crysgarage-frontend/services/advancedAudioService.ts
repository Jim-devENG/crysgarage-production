...
export interface AdvancedMasteringResponse {
  ...
  processed_audio_url?: string; // Backend field
  file_url?: string; // Backend field
  mastered_url?: string; // Backend field
  masteredUrl?: string; // Backend field (camelCase)
  processedUrl?: string; // Backend field (camelCase)
  lufs: number;
  format: string;
  duration: number;
  sample_rate: number;
...
class AdvancedAudioService {
  ...
  async masterAudioAdvanced(request: AdvancedMasteringRequest): Promise<AdvancedMasteringResponse> {
    try {
      ...
      // Required fields for master-advanced endpoint
      formData.append('file', request.file);                    // Audio file to be processed
      formData.append('genre', request.genre);                   // Audio genre for processing
      formData.append('tier', request.tier);                    // Processing tier
      formData.append('target_lufs', request.target_lufs.toString()); // Target loudness level
      formData.append('user_id', request.user_id);               // User identifier
      
      // Add compressor parameters
      formData.append('compressor_threshold', request.effects.compressor.threshold.toString()); // Compressor threshold
      formData.append('compressor_ratio', request.effects.compressor.ratio.toString());         // Compressor ratio
      formData.append('compressor_attack', request.effects.compressor.attack.toString());       // Compressor attack time
      formData.append('compressor_release', request.effects.compressor.release.toString());   // Compressor release time
      formData.append('compressor_enabled', request.effects.compressor.enabled.toString());   // Compressor enable flag
      
      // Add stereo widener parameters
      formData.append('stereo_width', request.effects.stereo_widener.width.toString());         // Stereo width value
      formData.append('stereo_enabled', request.effects.stereo_widener.enabled.toString());   // Stereo widener enable flag
      
      // Add loudness parameters
      formData.append('loudness_gain', request.effects.loudness.gain.toString());              // Loudness gain value
      formData.append('loudness_enabled', request.effects.loudness.enabled.toString());       // Loudness enable flag
      
      // Add limiter parameters
      formData.append('limiter_threshold', request.effects.limiter.threshold.toString());     // Limiter threshold
      formData.append('limiter_ceiling', request.effects.limiter.ceiling.toString());         // Limiter ceiling
      formData.append('limiter_enabled', request.effects.limiter.enabled.toString());         // Limiter enable flag
