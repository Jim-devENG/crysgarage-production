...
export function ProfilePage({ onNavigate }: ProfilePageProps) {
  ...
  // Check tier-specific access restrictions
  const isFreeTierRestricted = (user?.tier === 'free' || user?.tier === 'sapphire') && ((user?.free_credits ?? 0) < 1);
  const isAdvancedTierRestricted = (user?.tier === 'advanced' || user?.tier === 'jasper') && ((user?.advanced_credits ?? 0) < 1);
  const isRestricted = isFreeTierRestricted || isAdvancedTierRestricted;
  ...
  }, [user?.id, refreshUser]); // Refresh when user.id changes or component mounts
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {/* Restriction notice for tiers without tier-specific credits */}
        {isRestricted && (
          <div className="p-4 mb-6 rounded-lg border border-red-500/20 bg-red-500/10 text-red-300 flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-sm">
                {isFreeTierRestricted && "You need at least 1 Free tier credit to access the Free tier dashboard. Purchase Free tier credits to continue."}
                {isAdvancedTierRestricted && "You need at least 1 Advanced tier credit to access the Advanced tier dashboard. Purchase Advanced tier credits to continue."}
              </div>
              <div className="mt-3">
                <Button onClick={() => onNavigate('billing')} className="bg-crys-gold hover:bg-crys-gold/90 text-crys-black">
                  <CreditCard className="w-4 h-4 mr-2" /> 
                  {isFreeTierRestricted && "Purchase Free Tier Credits"}
                  {isAdvancedTierRestricted && "Purchase Advanced Tier Credits"}
                </Button>
              </div>
            </div>
          </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-crys-light-grey">Free Tier Credits</span>
                      <span className="text-crys-white font-semibold">{user.free_credits ?? 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-crys-light-grey">Advanced Tier Credits</span>
                      <span className="text-crys-white font-semibold">{user.advanced_credits ?? 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-crys-light-grey text-xs">Professional Tier</span>
                      <span className="text-crys-light-grey text-xs italic">Pay as you use</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
