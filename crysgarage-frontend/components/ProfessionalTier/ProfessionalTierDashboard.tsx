...
import { DEV_MODE, logDevAction, isDevModeActive } from '../../utils/secureDevMode';
...
const ProfessionalTierDashboard: React.FC<ProfessionalTierDashboardProps> = ({ onDownloadAttempt }) => {
  ...
  // CRITICAL: Restore state after payment redirect
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const paymentSuccess = urlParams.get('payment') === 'success';
    const downloadParam = urlParams.get('download') === 'true';
    
    if (paymentSuccess && downloadParam) {
      console.log('🔄 Payment success detected, restoring state...');
      // Restore processed audio state from localStorage
      try {
        const savedState = localStorage.getItem('professional_tier_download_state');
        if (savedState) {
          const state = JSON.parse(savedState);
          console.log('📦 Restored state from localStorage:', state);
          
          if (state.masteredAudioUrl || state.masteredRemoteUrl) {
            // Restore mastered audio URL (prefer remote URL for download)
            if (state.masteredRemoteUrl) {
              setMasteredRemoteUrl(state.masteredRemoteUrl);
              setMasteredAudioUrl(state.masteredRemoteUrl); // Use remote URL for download
            } else if (state.masteredAudioUrl) {
              setMasteredAudioUrl(state.masteredAudioUrl);
              setMasteredRemoteUrl(state.masteredAudioUrl);
            }
            
            if (state.processedFileId) {
              setProcessedFileId(state.processedFileId);
            }
            
            if (state.uploadedFile) {
              // Restore uploaded file metadata (File object can't be restored, but we have the URL)
              setUploadedFile({
                id: state.uploadedFile.id || `file_${Date.now()}`,
                name: state.uploadedFile.name || 'audio.wav',
                size: state.uploadedFile.size || 0,
                file: new File([], state.uploadedFile.name || 'audio.wav'), // Placeholder File
                url: state.masteredRemoteUrl || state.masteredAudioUrl || ''
              });
            }
            ...
            if (state.downloadFormat) {
              setDownloadFormat(state.downloadFormat);
            }
            
            if (state.sampleRate) {
              setDownloadSampleRate(state.sampleRate);
            }
            
            // CRITICAL: Switch to download tab so DownloadStep component is rendered
            setActiveTab('download');
            console.log('✅ State restored, switched to download tab');
          } else {
            console.warn('⚠️ No mastered audio URL in saved state');
          }
        } else {
          console.warn('⚠️ No saved state found in localStorage');
        }
      } catch (error) {
        console.error('❌ Failed to restore state after payment:', error);
      }
    }
  }, []);
  ...
  // Initialize audio context - resume on user gesture to avoid autoplay policy
  useEffect(() => {
    if (typeof window !== 'undefined' && !audioContextRef.current) {
      audioContextRef.current = new (window.AudioContext || (window as any).webkitAudioContext)();
      console.log('Audio context initialized for instant effects');
      
      // Resume AudioContext on first user interaction (click/touch)
  ...
  const getGenreDescription = (name: string): string => {
    const descriptions: Record<string, string> = {
      ...
      'auto preset': 'Minimal Processing'
    };
  ...
  const getHardcodedGenrePreset = (genreName: string) => {
    const presets: Record<string, any> = {
      ...
      'auto preset': {
        eq_curve: { low_shelf: { freq: 100, gain: 0.0 }, low_mid: { freq: 300, gain: 0.0 }, mid: { freq: 1000, gain: 0.0 }, high_mid: { freq: 4000, gain: 0.0 }, high_shelf: { freq: 8000, gain: 0.0 } },
        compression: { ratio: 1.0, threshold: 0.0, attack: 0.0, release: 0.0 },
        target_lufs: 0.0
      }
    };
  ...
  const startMasteringProcess = async () => {
    ...
    console.log('🎵 PROFESSIONAL TIER: startMasteringProcess called');
    console.log('🎵 PROFESSIONAL TIER: DEV_MODE =', isDevActive);
    console.log('🎵 PROFESSIONAL TIER: User =', effectiveUser);
    console.log('🎵 PROFESSIONAL TIER: Credits =', effectiveUser?.credits);
    ...
    // Professional tier is completely free - bypass ALL credit checks
    console.log('🎯 Professional tier: Completely free - bypassing ALL credit validation.');
    if (effectiveUser) {
      effectiveUser.credits = Infinity;
      effectiveUser.tier = 'professional';
      // Update localStorage to ensure tier and credits are set correctly
      const updatedUser = { ...effectiveUser, tier: 'professional', credits: Infinity };
    ...
    try {
      ...
      // Keep remote URL for download and playback
      // CRITICAL: Direct file URLs are CORS-safe and playable directly
      // No need for proxy-download (deprecated) or blob conversion
      setMasteredRemoteUrl(result.url);
      
      // CRITICAL: Use direct file URL for audio player
      // Files at /files/{tier}/mastered_*.wav are publicly accessible and CORS-enabled
      // This avoids 503 errors from deprecated proxy-download endpoint
      if (result.url && result.url.includes('/files/') && result.url.endsWith('.wav')) {
        // URL is already a direct file URL - use it directly
        const isLocal = typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1');
        const baseUrl = isLocal ? 'http://localhost:8002' : 'https://crysgarage.studio';
        const directUrl = result.url.startsWith('/files') 
          ? `${baseUrl}${result.url}` 
          : result.url;
        
        // Enforce HTTPS
        const finalUrl = (window.location.protocol === 'https:' && directUrl.startsWith('http://') && 
        ...
        setMasteredAudioUrl(finalUrl);
        console.log('✅ Professional tier: Using direct file URL for audio player:', finalUrl);
        
        // CRITICAL: Save state for payment redirect recovery
        // This ensures state is preserved if user is redirected for payment
        try {
          const stateToSave = {
            masteredAudioUrl: result.url, // Use remote URL (not blob URL) for persistence
            masteredRemoteUrl: result.url,
            processedFileId: result.file_id || null,
            ...
            downloadFormat,
            sampleRate: downloadSampleRate,
            timestamp: Date.now()
          };
          localStorage.setItem('professional_tier_download_state', JSON.stringify(stateToSave));
          console.log('✅ Saved download state after mastering:', stateToSave);
        } catch (error) {
          console.error('Failed to save download state:', error);
        }
      } else {
        // Fallback: use result.url directly if it doesn't match expected pattern
        setMasteredAudioUrl(result.url);
        console.log('✅ Professional tier: Using result URL directly:', result.url);
        
        // Save state
        try {
          const stateToSave = {
            masteredAudioUrl: result.url,
            masteredRemoteUrl: result.url,
            processedFileId: result.file_id || null,
            uploadedFile: uploadedFile ? {
              id: uploadedFile.id,
  ...
  // Download handler - PROFESSIONAL TIER: Direct file download ONLY
  // CRITICAL: Professional tier files already exist as final WAV files
  // No proxy-download (deprecated, returns 410), no fetch(), no blob conversion
  // Direct browser download using <a> tag with download attribute
  const handleDownload = async () => {
    // CRITICAL: Professional tier files are at /files/professional/mastered_{file_id}.wav
    // These files already exist and are publicly accessible
    const urlForDownload = masteredRemoteUrl || masteredAudioUrl;
    
    if (!urlForDownload) {
      console.error('❌ ProfessionalTierDashboard: No mastered file URL available');
      setError('No file available for download. Please process the audio first.');
      return;
    }
    
    // CRITICAL: Extract file_id if we have processedFileId, otherwise use URL
    let directDownloadUrl: string;
    
    if (processedFileId) {
      // Construct direct file URL from file_id
      const baseUrl = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')
        ? 'http://localhost:8002'
        : 'https://crysgarage.studio';
      directDownloadUrl = `${baseUrl}/files/professional/mastered_${processedFileId}.wav`;
      console.log('✅ Professional tier: Using direct file URL from file_id:', directDownloadUrl);
    } else if (urlForDownload.includes('/files/') && urlForDownload.endsWith('.wav')) {
      // URL is already a direct file URL
      const baseUrl = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')
        ? 'http://localhost:8002'
        : 'https://crysgarage.studio';
      directDownloadUrl = urlForDownload.startsWith('/files') 
        ? `${baseUrl}${urlForDownload}` 
        : urlForDownload;
      console.log('✅ Professional tier: Using direct file URL from mastered URL:', directDownloadUrl);
    } else {
      console.error('❌ ProfessionalTierDashboard: Invalid file URL format:', urlForDownload);
      setError('Invalid file URL. Expected /files/professional/mastered_*.wav');
      return;
    }
    
    // CRITICAL: Enforce HTTPS (no mixed content)
    if (window.location.protocol === 'https:' && directDownloadUrl.startsWith('http://') && 
        !directDownloadUrl.includes('localhost') && !directDownloadUrl.includes('127.0.0.1')) {
      directDownloadUrl = directDownloadUrl.replace('http://', 'https://');
      console.warn('⚠️ HTTP URL detected on HTTPS page, forcing HTTPS:', directDownloadUrl);
    }
    
    // CRITICAL: Guard against HTTP URLs in production
    if (!directDownloadUrl.startsWith('https://') && 
        !directDownloadUrl.includes('localhost') && 
        !directDownloadUrl.includes('127.0.0.1')) {
      setError('Download URL must use HTTPS. HTTP is not allowed in production.');
      return;
    }
    
    try {
      console.log('🎵 Professional tier: Starting direct file download (no proxy, no fetch):', directDownloadUrl);
      
      // CRITICAL: Generate filename with format info
      ...
      const sampleRateLabel = downloadSampleRate === 48000 ? '48k' : '44k';
      const genreName = selectedGenre?.name?.toLowerCase().replace(/\s+/g, '_') || '';
      
      const filename = `${baseName}_garage_${genreName ? `${genreName}_` : ''}mastered_${bitDepth}_${sampleRateLabel}.${ext}`;
      
      // CRITICAL: Use <a> tag with download attribute - NO fetch(), NO blob conversion
      // This triggers a direct browser download, avoiding 503 errors and HTML responses
      ...
      console.log('✅ Professional tier: Download completed successfully:', filename);
    } catch (error) {
      console.error('❌ Professional tier: Download failed:', error);
        ...
        {activeTab === 'download' && masteredAudioUrl && (
          <div className="max-w-4xl mx-auto">
            <ComparisonPlayerFixed
              ...
              onDownload={() => {}} // Not used - DownloadStep handles its own downloads
