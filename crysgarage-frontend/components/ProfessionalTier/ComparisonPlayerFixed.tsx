...
const ComparisonPlayerFixed: React.FC<ComparisonPlayerProps> = ({
  ...
  const initializeWebAudio = async () => {
    ...
    try {
      ...
      // Resume AudioContext on first user interaction to avoid autoplay policy
  ...
  const crossfadeTo = async (targetSource: 'original' | 'mastered', duration: number = 0.2) => {
    if (!audioContextRef.current || !originalGainRef.current || !masteredGainRef.current) {
    ...
    // CRITICAL: Wait for audio to be ready before crossfading
    if (targetSource === 'mastered') {
      if (!masteredAudio) {
        return; // Silently return if not available
      }
      // Wait for audio to have enough data (readyState >= 3 = HAVE_FUTURE_DATA)
      if (masteredAudio.readyState < 3) {
        // Wait for canplaythrough event (readyState 4) for smooth playback
        await new Promise<void>((resolve) => {
          ...
          const onCanPlayThrough = () => {
            masteredAudio.removeEventListener('canplaythrough', onCanPlayThrough);
            masteredAudio.removeEventListener('canplay', onCanPlayThrough);
            masteredAudio.removeEventListener('error', onError);
            resolve();
          };
          const onError = () => {
            masteredAudio.removeEventListener('canplaythrough', onCanPlayThrough);
          ...
          masteredAudio.addEventListener('canplaythrough', onCanPlayThrough, { once: true });
          masteredAudio.addEventListener('canplay', onCanPlayThrough, { once: true });
          masteredAudio.addEventListener('error', onError, { once: true });
          
          // Force load if needed
          if (masteredAudio.readyState === 0) {
            masteredAudio.load();
          }
      ...
      // Ensure mastered audio is playing if we're currently playing
      if (isPlaying && masteredAudio.paused) {
        try {
    ...
    } else if (targetSource === 'original') {
      if (!originalAudio) {
        return; // Silently return if not available
      }
      // Wait for audio to have enough data
      if (originalAudio.readyState < 3) {
        await new Promise<void>((resolve) => {
          if (originalAudio.readyState >= 3) {
            resolve();
            return;
          ...
          const onCanPlayThrough = () => {
            originalAudio.removeEventListener('canplaythrough', onCanPlayThrough);
            originalAudio.removeEventListener('canplay', onCanPlayThrough);
          ...
          originalAudio.addEventListener('canplaythrough', onCanPlayThrough, { once: true });
          originalAudio.addEventListener('canplay', onCanPlayThrough, { once: true });
          originalAudio.addEventListener('error', onError, { once: true });
          
          // Force load if needed
          if (originalAudio.readyState === 0) {
            originalAudio.load();
          }
      ...
      // Ensure original audio is playing if we're currently playing
      if (isPlaying && originalAudio.paused) {
        try {
    ...
    // Sync current time between both audio elements before crossfading
    if (targetSource === 'mastered' && originalAudio && masteredAudio) {
      masteredAudio.currentTime = originalAudio.currentTime;
    } else if (targetSource === 'original' && originalAudio && masteredAudio) {
      originalAudio.currentTime = masteredAudio.currentTime;
    }
    ...
    console.log(`✅ Crossfade to ${targetSource} initiated`);
  };
  ...
  useEffect(() => {
    if (isInitializedRef.current) {
      ...
      // Ensure mastered audio is loaded when URL becomes available
      if (masteredAudioUrl && masteredAudioRef.current) {
        const masteredAudio = masteredAudioRef.current;
        console.log('Mastered audio URL available, ensuring load...');
        ...
        const handleError = (e: Event) => {
          console.error('Error loading mastered audio:', e);
        };
        
        const handleCanPlay = () => {
          console.log('Mastered audio ready to play');
        };
        
        masteredAudio.addEventListener('error', handleError, { once: true });
        masteredAudio.addEventListener('canplay', handleCanPlay, { once: true });
        
        return () => {
          masteredAudio.removeEventListener('error', handleError);
          masteredAudio.removeEventListener('canplay', handleCanPlay);
        };
      }
    }
  ...
  const switchToOriginal = async () => {
    ...
    // crossfadeTo now handles all the waiting and preparation
    await crossfadeTo('original');
  };

  // Switch to mastered
  const switchToMastered = async () => {
    // Ensure audio context is resumed
    if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
      await audioContextRef.current.resume();
    }
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="p-6 bg-crys-dark-light rounded-xl border border-crys-gold/20">
          <DownloadStep
            ...
            onDownload={() => {}} // Not used - DownloadStep has its own download handler
