...
export function PricingPage({ onSelectTier, onGoToDashboard }: PricingPageProps) {
  ...
  const startDirectPayment = async (tierId: string) => {
    try {
      const priceMap: Record<string, number> = { professional: 4.00, advanced: 25.00, free: 5.00 };
      ...
      // Emerald (Professional) tier: No payment upfront - allow dashboard access immediately
      // Payment happens only when user tries to download
      if (tierId === 'professional') {
        // Navigate to professional tier dashboard immediately (no payment required)
        // Payment will be required when user clicks download
        onGoToDashboard();
        return;
      }
  ...
  const pricingTiers = [
    ...
    },
    {
      ...
      name: "Emerald Studio",
      price: "$4.00",
      priceNGN: formatNGN(convertUSDToNGN(4.00).ngn),
      subtitle: "$4 per download",
      description: "Perfect for active producers and artists",
      features: [
        "Pay per download ($4 each)",
        ...
        "Download WAV/MP3/FLAC/AIFF",
        "Professional mastering quality"
      ],
      limitations: [
        "$4 payment required per download",
          ...
          {pricingTiers.map((tier) => (
            <Card 
              ...
              className={`
                ...
                ${tier.id === 'free' 
                  ? 'border-2 border-blue-500 bg-gradient-to-b from-blue-500/10 to-blue-500/5 hover:border-blue-400' 
                  : tier.id === 'professional'
                  ? 'border-2 border-green-500 bg-gradient-to-b from-green-500/10 to-green-500/5 hover:border-green-400'
                  : 'border-2 border-red-500 bg-gradient-to-b from-red-500/10 to-red-500/5 hover:border-red-400'
                }
                ${tier.popular 
                  ? 'border-crys-gold bg-gradient-to-b from-crys-gold/10 to-transparent shadow-lg shadow-crys-gold/20' 
                  : ''
                }
                bg-audio-panel-bg
              `}
            >
              ...
              <CardHeader className="text-center pb-2 pt-6">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center mx-auto mb-2 ${
                  tier.id === 'free' ? 'bg-blue-500/20 text-blue-400' :
                  tier.id === 'professional' ? 'bg-green-500/20 text-green-400' :
