...
export function BillingPage({ onNavigate }: BillingPageProps) {
  ...
  React.useEffect(() => {
    ...
    if (urlParams.get('payment') === 'success') {
      ...
      if (tier && credits) {
        ...
        // Navigate to the correct tier dashboard based on purchased tier
        // Map 'pro' to 'professional' for navigation, 'advanced' to 'advanced', 'free' to 'dashboard'
        const targetPage = tier === 'advanced' ? 'advanced' : (tier === 'pro' || tier === 'professional') ? 'professional' : 'dashboard';
        setTimeout(() => {
  ...
  const tierPricing = {
    free: { credits: 1, price: 5.00, name: 'Sapphire Studio' },
  ...
  const loadBillingData = async () => {
    ...
    try {
      ...
      const paymentMethodsResponse = await fetch(`${baseUrl}/payment-methods/${user.id}`);
      ...
      const invoicesResponse = await fetch(`${baseUrl}/invoices/${user.id}`);
      if (invoicesResponse.ok) {
        const invoicesData = await invoicesResponse.json();
        // Convert invoices to transactions format for display
        const invoiceTransactions = (invoicesData.invoices || []).map((invoice: any) => ({
          id: invoice.id,
          amount: invoice.amount,
          credits: invoice.credits,
          status: invoice.status,
          date: invoice.created_at,
          description: `${invoice.tier} Tier - ${invoice.credits} Credits`,
  ...
  const handleSaveCard = async (cardData: any) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/payment-methods/save`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(errorData.detail || 'Failed to save payment method');
      }
    } catch (error) {
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            {transaction.invoice_number && (
                              <p className="text-xs text-crys-light-grey mt-1">
                          ...
                          {transaction.invoice_number && (
                            <Button
                              ...
                              onClick={() => {
                                ...
                                a.href = url;
                                a.download = `invoice-${transaction.invoice_number}.json`;
                                a.click();
