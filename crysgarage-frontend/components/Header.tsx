...
import { DEV_MODE, isDevModeActive } from "../utils/secureDevMode";
...
export function Header({ 
  ...
  // Only check for actual dev mode, not based on credits value (which could be undefined/null)
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="hidden lg:block">
                  {isDevUser ? (
                    <span className="px-2 py-1 rounded bg-green-500/10 border border-green-500/20 text-green-400 text-xs">Dev Mode</span>
                  ) : (
                    <CreditDisplay 
                      onPurchaseClick={() => onNavigate('billing')}
