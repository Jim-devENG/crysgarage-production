...
// Dev Mode detection
// PRODUCTION-SAFE: Only enabled on localhost during development
// This prevents accidental dev mode activation in production via /dev route or localStorage
const envDev = import.meta.env.VITE_DEV_MODE === 'true';
const isLocalhost = typeof window !== 'undefined' && window.location && (
  window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
);

// SECURITY: Dev mode ONLY works on localhost with env flag
// URL triggers (/dev, ?dev=1) and localStorage bypass disabled in production
