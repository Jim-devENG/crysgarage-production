...
export function LandingPage({ onGetStarted, onTryMastering }: LandingPageProps) {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Partners Section */}
      <section className="bg-crys-charcoal/30 relative pt-12 pb-12">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <Badge variant="secondary" className="bg-crys-gold/20 text-crys-gold mb-4">Partners</Badge>
            <h2 className="text-3xl md:text-4xl font-bold text-crys-white">Trusted by Our Partners</h2>
            <p className="text-crys-light-grey mt-2">We collaborate with studios, distributors, and creative communities</p>
          </motion.div>
          ...
          >
            {[
              { name: 'Partner One', src: '/Image7.png' },
              { name: 'Partner Two', src: '/Image8.png' },
              { name: 'Partner Three', src: '/Image9.png' },
              { name: 'Partner Four', src: '/Jusi.png' },
              { name: 'Partner Five', src: '/Jim.png' },
              { name: 'Partner Six', src: '/CRG_Logo_svg.svg' },
            ].map((partner, i) => (
              <motion.div
                key={partner.name + i}
                className="flex items-center justify-center bg-audio-panel-bg/40 border border-crys-graphite/60 rounded-xl h-20 p-4 hover:border-crys-gold/50 transition-colors"
                variants={itemVariants}
              >
                <img
                  src={partner.src}
                  alt={partner.name}
                  className="max-h-10 object-contain opacity-90 hover:opacity-100 transition-opacity"
                  loading="lazy"
                />
              </motion.div>
            ))}
