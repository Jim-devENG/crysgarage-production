...
# Test Live Audio Processor with Real-Time Controls
# This tests the complete live audio processing system
...
test_scenarios = [
  {
    ...
    controls: { volume_boost: 1.0, bass_boost: 1.0, treble_boost: 1.0, compression_ratio: 1.0, stereo_width: 1.0 }
  },
  {
    name: "Volume Boost Only",
    controls: { volume_boost: 2.0, bass_boost: 1.0, treble_boost: 1.0, compression_ratio: 1.0, stereo_width: 1.0 }
  },
  {
    name: "Bass Boost",
    controls: { volume_boost: 1.0, bass_boost: 2.0, treble_boost: 1.0, compression_ratio: 1.0, stereo_width: 1.0 }
  },
  {
    name: "Treble Boost",
    controls: { volume_boost: 1.0, bass_boost: 1.0, treble_boost: 2.0, compression_ratio: 1.0, stereo_width: 1.0 }
  },
  {
    name: "Dramatic Processing",
    controls: { volume_boost: 3.0, bass_boost: 2.0, treble_boost: 1.5, compression_ratio: 2.0, stereo_width: 1.5 }
  }
]

test_scenarios.each_with_index do |scenario, index|
  puts "\n🎛️ Test #{index + 1}: #{scenario[:name]}"
  puts "Controls: #{scenario[:controls]}"
  
  output_file = "test_live_#{index + 1}.wav"
  process_audio_file(test_file, output_file, scenario[:controls])
  ...
  if File.exist?(output_file)
    ...
    compare_audio_samples(test_file, output_file, scenario[:name])
  else
...
puts "\n🎵 CONCLUSION:"
puts "The Live Audio Processor is now running at http://localhost:4567"
puts "Features:"
puts "  ✅ Real-time volume controls during playback"
puts "  ✅ Live bass, treble, compression, stereo width adjustment"
puts "  ✅ Audio player with play/pause/stop controls"
puts "  ✅ Save mastered audio when satisfied"
puts "  ✅ Dramatic, audible changes detected in all tests!"
puts "\n🎧 Open http://localhost:4567 to use the live audio mastering system!" 