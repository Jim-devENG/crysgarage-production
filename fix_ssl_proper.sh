...
# Remove old SSL certificates if they exist
echo "🧹 Cleaning up old SSL certificates..."
rm -rf /etc/letsencrypt/live/crysgarage.studio 2>/dev/null || true
rm -rf /etc/letsencrypt/archive/crysgarage.studio 2>/dev/null || true

# Get new SSL certificate
echo "🔒 Obtaining new SSL certificate..."
certbot certonly --standalone -d crysgarage.studio -d www.crysgarage.studio --non-interactive --agree-tos --email admin@crysgarage.studio
