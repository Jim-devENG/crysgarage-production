...
# from .pitch_correction import PitchCorrection  # Removed 444 Hz tuning
...
class MLMasteringEngine:
    ...
    def __init__(self):
        ...
        # self.pitch_corrector = PitchCorrection()  # Removed 444 Hz tuning
    ...
    def _load_tier_settings(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "Kwaito": {
                ...
                "target_lufs": -14.0                             # -14 LUFS (streaming standard)
            },
            "Gqom": {
                "eq_curve": {
    ...
    async def process_audio(
        ...
        try:
            ...
            # Note: Pitch correction is applied later in the mastering chain
            ...
            # Use genre-based target LUFS instead of fixed value
            genre_target_lufs = preset.get("target_lufs", -10.0)
            # Override with genre-based LUFS if provided, otherwise use the passed target_lufs
            effective_target_lufs = genre_target_lufs if genre_target_lufs != -14.0 else target_lufs
            
            logger.info(f"Processing with tier: {tier}, quality: {tier_config['processing_quality']}")
            logger.info(f"Genre-based target LUFS: {effective_target_lufs} dB (from {genre} preset)")
            ...
            # 5. Comprehensive Normalization with Brick Wall Limiter (within -8 and -10 pegged)
    ...
    def _apply_standard_stereo_widening(self, audio_data: np.ndarray, tier_config: Dict[str, Any]) -> np.ndarray:
        """Apply standard stereo widening for Professional+ tiers"""
        try:
            logger.info("Applying standard stereo widening")
            ...
            # Standard stereo width based on tier
            if tier_config["processing_quality"] == "standard":
                width = 1.2
            else:  # premium
                width = 1.4
            ...
            logger.error(f"Standard stereo widening failed: {e}")
            return audio_data
    ...
    def _apply_genre_preset_compression(self, audio_data: np.ndarray, sample_rate: int, genre: str) -> np.ndarray:
        """Apply ONLY the genre preset compression - no additional processing"""
        try:
            preset = self.genre_presets.get(genre, self.genre_presets["Pop"])
            compression = preset["compression"]
            
            logger.info(f"Applying genre preset compression for {genre}")
            
            processed = audio_data.copy()
            
            # Apply simple compression based on genre preset
            ...
            for channel in range(processed.shape[0]):
                ...
                for i, sample in enumerate(channel_data):
                    ...
                    if abs(sample) > envelope:
                        envelope += (abs(sample) - envelope) * (1 - np.exp(-1 / (attack * sample_rate)))
                    else:
                        envelope += (abs(sample) - envelope) * (1 - np.exp(-1 / (release * sample_rate)))
            ...
            logger.error(f"Genre preset compression failed: {e}")
            return audio_data

    def _apply_genre_preset_stereo_width(self, audio_data: np.ndarray, genre: str) -> np.ndarray:
        """Apply ONLY the genre preset stereo width - no additional processing"""
        try:
            preset = self.genre_presets.get(genre, self.genre_presets["Pop"])
            stereo_width = preset["stereo_width"]
            
            logger.info(f"Applying genre preset stereo width for {genre}: {stereo_width}")
            ...
            logger.error(f"Genre preset stereo width failed: {e}")
            return audio_data

    def _apply_genre_preset_lufs(self, audio_data: np.ndarray, sample_rate: int, target_lufs: float) -> np.ndarray:
        """Apply ONLY the genre preset target LUFS - simple normalization"""
        try:
            logger.info(f"Applying genre preset LUFS: {target_lufs} dB")
            
            processed = audio_data.copy()
            
            # Simple LUFS calculation and normalization
            # Calculate RMS
            rms = np.sqrt(np.mean(processed ** 2))
            rms_db = 20 * np.log10(rms + 1e-10)
            
            # Calculate gain needed to reach target LUFS
            # Approximate LUFS as RMS for simplicity
            ...
            logger.error(f"Genre preset LUFS failed: {e}")
            return audio_data

    def _apply_genre_based_eq(self, audio_data: np.ndarray, sample_rate: int, genre: str, tier_config: Dict[str, Any]) -> np.ndarray:
        ...
        try:
            ...
            # CRITICAL: Gain compensation after EQ to prevent gain accumulation
            # Calculate total EQ gain and compensate
            total_eq_gain = (low_shelf["gain"] + low_mid["gain"] + mid["gain"] + 
                           high_mid["gain"] + high_shelf["gain"])
            
            if total_eq_gain > 0:
                # Compensate for positive EQ gain to maintain unity gain
                compensation_gain = 10 ** (-total_eq_gain / 20)  # Convert dB to linear
                processed = processed * compensation_gain
                logger.info(f"EQ gain compensation applied: -{total_eq_gain:.1f}dB to prevent gain accumulation")
            
            logger.info(f"Genre-based EQ applied for {genre} (total EQ gain: {total_eq_gain:.1f}dB)")
            return processed
            
        except Exception as e:
            logger.warning(f"Genre-based EQ failed: {e}")
            return audio_data

    def _apply_genre_based_compression(self, audio_data: np.ndarray, sample_rate: int, genre: str, tier_config: Dict[str, Any]) -> np.ndarray:
        """Apply compression based on genre preset"""
        try:
            preset = self.genre_presets.get(genre, self.genre_presets["Pop"])
            compression = preset["compression"]
            
            logger.info(f"Applying genre-based compression for {genre}")
            
            processed = audio_data.copy()
            
            # Apply compression settings from genre preset
            threshold = compression["threshold"]
            ratio = compression["ratio"]
            attack = compression["attack"]
            release = compression["release"]
            ...
            # Simple compression implementation
            processed = self._apply_simple_compression(processed, threshold, ratio, attack, release)
            
            logger.info(f"Genre-based compression applied for {genre} - Threshold: {threshold}dB, Ratio: {ratio}:1")
            return processed
            
        except Exception as e:
            logger.warning(f"Genre-based compression failed: {e}")
            return audio_data
        ...
        try:
            ...
            if audio_data.ndim == 2:  # Stereo
                for ch in range(audio_data.shape[0]):
                    ...
                    above_threshold = np.abs(channel) > threshold_linear
                    
                    if np.any(above_threshold):
                        # Apply compression
                        compressed = np.where(
                            above_threshold,
                            np.sign(channel) * (threshold_linear + (np.abs(channel) - threshold_linear) / ratio),
                            channel
                        )
                        processed[ch] = compressed
            else:  # Mono
                above_threshold = np.abs(processed) > threshold_linear
                if np.any(above_threshold):
                    compressed = np.where(
                        above_threshold,
                        np.sign(processed) * (threshold_linear + (np.abs(processed) - threshold_linear) / ratio),
                        processed
                    )
            ...
            logger.warning(f"Simple compression failed: {e}")
            return audio_data

    def _log_gain_staging(self, stage_name: str, audio_data: np.ndarray) -> None:
        """Professional gain staging monitoring"""
        try:
            peak = np.max(np.abs(audio_data))
            rms = self._calculate_rms(audio_data)
            peak_db = 20 * np.log10(peak + 1e-10)
            logger.info(f"GAIN STAGING: {stage_name} - Peak: {peak_db:.1f}dB, RMS: {rms:.1f}dB")
        except Exception as e:
            logger.warning(f"Gain staging logging failed: {e}")
    ...
    def _apply_final_stereo_widener(self, audio_data: np.ndarray, genre: str, tier_config: Dict[str, Any]) -> np.ndarray:
        """Apply final stereo widener to enhance stereo image after all processing"""
        try:
            if audio_data.shape[0] != 2:
                return audio_data
            
            logger.info("Applying final stereo widener")
            
            # Get genre preset for stereo width
            preset = self.genre_presets.get(genre, self.genre_presets["Pop"])
            base_width = preset.get("stereo_width", 1.0)
            
            # Apply additional final widening based on tier
            tier_multiplier = {
                "Free": 1.1,           # 10% additional widening
                "Professional": 1.15,  # 15% additional widening
                "Pro": 1.15,           # 15% additional widening
                "Advanced": 1.2        # 20% additional widening
            }
            
            tier = tier_config.get("processing_quality", "basic")
            if tier == "basic":
                tier_key = "Free"
            elif tier == "standard":
            ...
            final_width = base_width * tier_multiplier.get(tier_key, 1.1)
            
            # Ensure width doesn't exceed reasonable limits
            final_width = min(final_width, 1.5)  # Max 50% widening
            
            if final_width <= 1.0:
                logger.info(f"Final stereo widener: No additional widening needed (width: {final_width:.2f})")
                return audio_data
            
            # Apply final stereo widening
            left = audio_data[0]
            right = audio_data[1]
            
            # Mid-side processing for final widening
            mid = (left + right) / 2
            side = (left - right) / 2
            
            # Apply final width enhancement
            side *= final_width
            ...
            # Apply subtle high-frequency enhancement for stereo clarity
            # This helps maintain stereo definition after all processing
            if tier_key in ["Professional", "Pro", "Advanced"]:
                # Apply gentle high-frequency stereo enhancement
                enhanced_left, enhanced_right = self._apply_stereo_clarity_enhancement(new_left, new_right)
                processed = np.stack([enhanced_left, enhanced_right])
            else:
                processed = np.stack([new_left, new_right])
            
            logger.info(f"Final stereo widener applied: {base_width:.2f} → {final_width:.2f} width")
            return processed
            
        except Exception as e:
            logger.warning(f"Final stereo widener failed: {e}")
            return audio_data

    def _apply_stereo_clarity_enhancement(self, left: np.ndarray, right: np.ndarray) -> tuple:
        """Apply subtle high-frequency enhancement for stereo clarity"""
        try:
            # Simple high-frequency stereo enhancement
            # This helps maintain stereo definition in the final mix
            
            # Calculate difference signal (stereo information)
            stereo_diff = left - right
            
            # Apply gentle high-frequency boost to stereo difference
            # This enhances stereo clarity without affecting mono compatibility
            enhancement_factor = 0.1  # 10% enhancement
            enhanced_diff = stereo_diff * (1 + enhancement_factor)
            
            # Reconstruct left and right with enhanced stereo information
            mid = (left + right) / 2
            enhanced_left = mid + enhanced_diff / 2
            enhanced_right = mid - enhanced_diff / 2
            
            return enhanced_left, enhanced_right
            
        except Exception as e:
            logger.warning(f"Stereo clarity enhancement failed: {e}")
            return left, right

    def _apply_final_loudness_safety(self, audio_data: np.ndarray, sample_rate: int, target_lufs: float) -> np.ndarray:
        """Apply final loudness safety check to ensure audio is not too loud (genre-based)"""
        try:
            logger.info("Applying final loudness safety check")
            
            # Calculate final LUFS and RMS
            final_lufs = self._calculate_simple_lufs(audio_data, sample_rate)
            final_rms = self._calculate_rms(audio_data)
            final_peak = np.max(np.abs(audio_data))
            
            logger.info(f"Final measurements - LUFS: {final_lufs:.2f} dB, RMS: {final_rms:.2f} dB, Peak: {final_peak:.3f}")
            
            # Safety limits (genre-based with safety cap)
            max_safe_lufs = min(target_lufs, -8.0)  # Use genre target, but cap at -8 dB for safety
            max_safe_rms = -10.0   # Maximum safe RMS
            ...
            # Check LUFS safety
            if final_lufs > max_safe_lufs:
                lufs_gain_db = max_safe_lufs - final_lufs
                lufs_gain_linear = 10 ** (lufs_gain_db / 20)
                processed = processed * lufs_gain_linear
                logger.info(f"LUFS safety applied: {final_lufs:.2f} → {max_safe_lufs:.2f} dB")
                safety_applied = True
            
            # Check RMS safety
            new_rms = self._calculate_rms(processed)
            if new_rms > max_safe_rms:
                rms_gain_db = max_safe_rms - new_rms
                rms_gain_linear = 10 ** (rms_gain_db / 20)
                processed = processed * rms_gain_linear
                logger.info(f"RMS safety applied: {new_rms:.2f} → {max_safe_rms:.2f} dB")
                safety_applied = True
            
            # Check peak safety
            new_peak = np.max(np.abs(processed))
            if new_peak > max_safe_peak:
                peak_gain_linear = max_safe_peak / new_peak
                processed = processed * peak_gain_linear
                logger.info(f"Peak safety applied: {new_peak:.3f} → {max_safe_peak:.3f}")
                safety_applied = True
            
            if safety_applied:
                # Final measurements after safety
                final_lufs_after = self._calculate_simple_lufs(processed, sample_rate)
                final_rms_after = self._calculate_rms(processed)
                final_peak_after = np.max(np.abs(processed))
                logger.info(f"Final safety results - LUFS: {final_lufs_after:.2f} dB, RMS: {final_rms_after:.2f} dB, Peak: {final_peak_after:.3f}")
            else:
                logger.info("Final loudness safety: No adjustments needed")
            
            return processed
            
        except Exception as e:
            logger.warning(f"Final loudness safety failed: {e}")
            return audio_data
    ...
    def _apply_comprehensive_normalization_with_brick_wall(self, audio_data: np.ndarray, sample_rate: int, target_lufs: float) -> np.ndarray:
        """
        Apply comprehensive normalization with brick wall limiter
        RMS should be between -12 and -10, LUFS should not go beyond -10
        """
        try:
            logger.info("Applying comprehensive normalization with brick wall limiter")
            
            # Step 1: Initial LUFS Normalization
            processed = self._normalize_lufs(audio_data, sample_rate, target_lufs)
            
            # Step 2: Apply Brick Wall Limiter with RMS and LUFS constraints
            processed = self._apply_brick_wall_limiter(processed, sample_rate, target_lufs)
            
            # Step 3: Final safety check and soft limiting
            processed = self._apply_soft_limiting(processed)
            
            logger.info("Comprehensive normalization with brick wall limiter completed")
            return processed
            
        except Exception as e:
            logger.error(f"Comprehensive normalization with brick wall limiter failed: {e}")
            return audio_data

    def _apply_brick_wall_limiter(self, audio_data: np.ndarray, sample_rate: int, target_lufs: float) -> np.ndarray:
        """
        Apply brick wall limiter with RMS between -12 and -10, LUFS based on genre target
        """
        try:
            logger.info("Applying brick wall limiter")
            
            # Calculate current RMS and LUFS
            current_rms = self._calculate_rms(audio_data)
            current_lufs = self._calculate_simple_lufs(audio_data, sample_rate)
            
            logger.info(f"Current RMS: {current_rms:.2f} dB, Current LUFS: {current_lufs:.2f} dB")
            
            # Target RMS range: -12 to -10 dB (more conservative)
            target_rms_min = -12.0
            target_rms_max = -10.0
            # Use genre-based target LUFS, but ensure it doesn't exceed -8 dB for safety
            target_lufs_max = min(target_lufs, -8.0)
            
            # Calculate RMS gain adjustment
            if current_rms < target_rms_min:
                # Too quiet, boost to minimum target
                rms_gain_db = target_rms_min - current_rms
                rms_gain_linear = 10 ** (rms_gain_db / 20)
                processed = audio_data * rms_gain_linear
                logger.info(f"RMS boost: {current_rms:.2f} → {target_rms_min:.2f} dB")
            elif current_rms > target_rms_max:
                # Too loud, reduce to maximum target
                rms_gain_db = target_rms_max - current_rms
                rms_gain_linear = 10 ** (rms_gain_db / 20)
                processed = audio_data * rms_gain_linear
                logger.info(f"RMS reduction: {current_rms:.2f} → {target_rms_max:.2f} dB")
            else:
                # RMS is in target range
                processed = audio_data.copy()
                logger.info(f"RMS in target range: {current_rms:.2f} dB")
            
            # Recalculate LUFS after RMS adjustment
            new_lufs = self._calculate_simple_lufs(processed, sample_rate)
            
            # Apply LUFS constraint (not beyond -8)
            if new_lufs > target_lufs_max:
                lufs_gain_db = target_lufs_max - new_lufs
                lufs_gain_linear = 10 ** (lufs_gain_db / 20)
                processed = processed * lufs_gain_linear
                logger.info(f"LUFS constraint applied: {new_lufs:.2f} → {target_lufs_max:.2f} dB")
            
            # Apply brick wall limiting to prevent any clipping
            max_level = 0.989  # Brick wall at -0.1 dB
            peak = np.max(np.abs(processed))
            ...
            if peak > max_level:
                ...
                gain_reduction = max_level / peak
                processed = processed * gain_reduction
                logger.info(f"Brick wall limiting applied: peak {peak:.3f} → {max_level:.3f}")
            
            # Final measurements
            final_rms = self._calculate_rms(processed)
            final_lufs = self._calculate_simple_lufs(processed, sample_rate)
            final_peak = np.max(np.abs(processed))
            
            logger.info(f"Brick wall limiter results - RMS: {final_rms:.2f} dB, LUFS: {final_lufs:.2f} dB, Peak: {final_peak:.3f}")
            
            return processed
            
        except Exception as e:
            logger.warning(f"Brick wall limiter failed: {e}")
            return audio_data
    ...
    def _calculate_rms(self, audio_data: np.ndarray) -> float:
        ...
        try:
            ...
            return -20.0  # Default safe value

    def _normalize_lufs(self, audio_data: np.ndarray, sample_rate: int, target_lufs: float) -> np.ndarray:
        """Normalize audio to target LUFS level, but never exceed -10 dB"""
        try:
            # Calculate current LUFS (simplified)
            current_lufs = self._calculate_simple_lufs(audio_data, sample_rate)
            
            # Enforce maximum LUFS limit of -10 dB (more conservative)
            max_lufs = -10.0
            effective_target_lufs = min(target_lufs, max_lufs)
            ...
            # Verify final LUFS doesn't exceed -8 dB
            final_lufs = self._calculate_simple_lufs(processed, sample_rate)
            if final_lufs > max_lufs:
                # Apply additional limiting to ensure -8 dB maximum
                additional_gain_db = max_lufs - final_lufs
                additional_gain_linear = 10 ** (additional_gain_db / 20)
                processed = processed * additional_gain_linear
                final_lufs = self._calculate_simple_lufs(processed, sample_rate)
                logger.info(f"LUFS normalization with limit: {current_lufs:.1f} → {final_lufs:.1f} LUFS (limited to -8 dB max)")
            else:
                logger.info(f"LUFS normalization: {current_lufs:.1f} → {final_lufs:.1f} LUFS")
    ...
    async def preview_genre_effects(
        ...
        try:
            ...
            # Pitch correction removed - no longer tuning to 444.0 Hz
    ...
    #     """
    #     REMOVED: 444 Hz tuning functionality
    #     """
    #     return audio_data  # No pitch shifting
