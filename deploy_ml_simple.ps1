# Simple ML Pipeline Deployment Script
param(
...
Write-Host "Starting ML Pipeline VPS Deployment..." -ForegroundColor Cyan
...
Write-Host "Configuring Nginx for ML pipeline..." -ForegroundColor Yellow
ssh $SSH_OPTS $VPS_USER@$VPS_HOST @"
...
curl -s https://crysgarage.studio/ml-pipeline/api/health
echo "Health endpoint test completed"
curl -s https://crysgarage.studio/ml-pipeline/api/upload-audio
echo "Upload endpoint test completed"
"@
